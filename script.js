...
map.on("load", function () {
	...
	fetch(geojsonUrl)
		...
		.then((data) => {
			...
			// fonction de selction des types renovation de batiments en lien avec la commune
			filterRenov.onchange = () => {
				const selectedRenov = Array.from(
					filterRenov.selectedOptions,
					(option) => option.value
				);
				const selectedCommunes = Array.from(
					communeSelect.selectedOptions,
				...
				if (selectedRenov.length > 0) {
					if (selectedCommunes.length > 0) {
						...
						newGeoJSON5.features = filteredFeatures.filter((feature) =>
							selectedRenov.includes(feature.properties.RENOVATION)
						);
					} else {
						// Afficher tous les types de bâtiments car aucune commune n'est sélectionnée
					...
					// Afficher tous les bâtiments de base car aucun type n'est sélectionné
					newGeoJSON5.features = [...geojson.features];
	...
	$(document).ready(function () {
		var liste_choices4 = [];
		var currentSelections = [];
		var options4 = ["Oui", "Non", "à définir"];
...
fetch("./data/BD_conso.json")
	...
	.then((response) => {
		...
		function attachUpdateEventListeners(buttonIds) {
			buttonIds.forEach(function(id) {
			  document.querySelector("#" + id).addEventListener("click", updateChart);
			});
		  }
		  
		  // Utilisez la fonction pour attacher le gestionnaire d'événements à plusieurs boutons
		  attachUpdateEventListeners(["btn-choix", "btn-choix2", "btn-choix4","btn-choix5"]);
	...
	fetch("./data/BD_conso.json")
		...
		.then((jsonData) => {
			...
			function attachEventListeners(buttonIds) {
				buttonIds.forEach(function(id) {
				  document.querySelector("#" + id).addEventListener("click", updateChartData);
				});
			  }
			  
			  // Utilisez la fonction pour attacher le gestionnaire d'événements à plusieurs boutons
			  attachEventListeners(["btn-choix", "btn-choix2", "btn-choix4", "btn-choix5"]);
